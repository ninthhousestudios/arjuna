...
void main(List<String> args) {
  ...
  // The universe still needing a witness. Start with all 168 slots.
  final needHealthy = _allBeings().toSet();
  final needUnhealthy = _allBeings().toSet();
  ...
  for (final corpusDir in corpora) {
    ...
    for (final file in files) {
      ...
      final witness = _rankChart(file, label, facade, options);
      if (witness == null) {
        chartsFailed++;
        continue;
      }
  ...
  stderr.writeln(
    ...
    '${_allBeings().length * 2 - needHealthy.length - needUnhealthy.length} '
    'of ${_allBeings().length * 2} slots.',
  );
}
...
Set<BeingKey> _activated(BeingHealth r) => {
...
String _writeReport({
  ...
  final all = _allBeings();
  final out = StringBuffer()
    ..writeln('# Being health extremes — witness charts')
    ..writeln()
    ..writeln(
      'Generated by `tool/bin/find_being_extremes.dart`. For each of the '
      '${all.length} beings, one chart in which it is *the healthiest* being '
      'and one in which it is *the unhealthiest*, chosen to use the fewest '
      'charts overall (greedy set cover). A being is as healthy as the planet '
      'activating it; the beings of a chart\'s rank-1 planet are its '
      'healthiest, those of its last-rank planet its unhealthiest.',
    )
    ..writeln()
    ..writeln(
...
List<BeingKey> _allBeings() => [
  for (var sign = 1; sign <= 12; sign++)
    for (final type in BeingType.values) BeingKey(sign, type),
];
