...
void main() {
  ...
  const quadrant = {
    1: [2, 5, 8], 2: [7, 10, 1], 3: [6, 9, 12],
    4: [5, 8, 11], 5: [10, 1, 4], 6: [9, 12, 3],
    7: [8, 11, 2], 8: [1, 4, 7], 9: [12, 3, 6],
    10: [11, 2, 5], 11: [4, 7, 10], 12: [3, 6, 9],
  };

  const element = {
    1: [2, 8, 11], 2: [7, 4, 1], 3: [6, 9, 12],
    4: [5, 2, 11], 5: [7, 10, 4], 6: [9, 12, 3],
    7: [8, 11, 5], 8: [1, 10, 7], 9: [12, 3, 6],
    10: [11, 2, 8], 11: [4, 1, 10], 12: [3, 6, 9],
  };

  const conventional = {
    1: [5, 8, 11], 2: [4, 7, 10], 3: [6, 9, 12],
    4: [8, 11, 2], 5: [7, 10, 1], 6: [9, 12, 3],
    7: [11, 2, 5], 8: [10, 1, 4], 9: [12, 3, 6],
  ...
  group('doesAspect — default mode is quadrant', () {
    ...
    test('sign 1 DOES aspect 11 in conventional', () {
      expect(
          RashiAspect.doesAspect(1, 11, RashiAspectMode.conventional), isTrue);
    });
  });

  group('doesAspect — modes differ where expected', () {
    test('sign 1: quadrant has 2 but not 11; conventional has 11 but not 2',
        () {
      expect(
          RashiAspect.doesAspect(1, 2, RashiAspectMode.quadrant), isTrue);
      expect(
          RashiAspect.doesAspect(1, 11, RashiAspectMode.quadrant), isFalse);
      expect(
          RashiAspect.doesAspect(1, 2, RashiAspectMode.conventional), isFalse);
      expect(
          RashiAspect.doesAspect(1, 11, RashiAspectMode.conventional), isTrue);
    });
    ...
    test('mode is respected', () {
      expect(
          RashiAspect.doesAspectWithOccupants(
              1, 11, true, RashiAspectMode.conventional),
          isTrue);
      expect(
          RashiAspect.doesAspectWithOccupants(
              1, 11, true, RashiAspectMode.quadrant),
          isFalse);
    });
  });
