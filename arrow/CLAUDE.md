...
| `arrow_swe` | Swiss Ephemeris facade | `SweFacade.calcAll()` → `EphSnapshot`; `calcBodies()` → `BodySnapshot` (positions only, 4-13x cheaper, for scan loops). Raw positions only. |
...
Run tests with `arrow/` as the working directory — `calc/test/vedic/avastha_golden_test.dart` resolves its fixtures by relative path and fails to load from anywhere else.

## Code generation

After any `dart run build_runner build`, run `dart run tool/bin/license_header.dart`. Codegen rewrites every `*.freezed.dart` and `*.g.dart` with its own header, stripping the SPDX and copyright lines; the script restores them and `calc/test/license_header_test.dart` fails until it has been run.

Note that `--delete-conflicting-outputs` matches the `Bash(*--delete*)` deny rule in `~/.claude/settings.json`, so agents cannot run that form — use plain `dart run build_runner build`.
