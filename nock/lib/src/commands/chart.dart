...
class ChartCommand extends Command<void> {
  ...
  Future<void> run() async {
    ...
    try {
      ...
      final response = await client.calculate(CalcRequest(
        jdUt: jd,
  ...
  void _printChart(EphSnapshot snapshot, double lat, double lon) {
    ...
    print('│  Location: ${lat.toStringAsFixed(4)}N, '
        '${lon.toStringAsFixed(4)}E');
    print('├─────────────────────────────────────────┤');
    print('│  Asc ${_fmtLon(snapshot.ascmc.ascendant)}  '
    ...
    for (var i = 0; i < snapshot.cusps.length; i++) {
      print('│  House ${(i + 1).toString().padLeft(2)}: ${_fmtLon(snapshot.cusps[i])}');
    }
