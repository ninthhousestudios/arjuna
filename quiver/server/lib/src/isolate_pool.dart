...
class _CalcRequest {
  ...
  List<Object> toMessage() =>
      [id, jdUt, locationJsonStr, optionsJsonStr, replyPort];
...
///
/// Each worker isolate creates its own [SwissEph] + [SweFacade] — swisseph.dart
/// is isolate-safe because each isolate gets independent C state via dart:ffi.
///
    ...
    for (var i = 0; i < size; i++) {
      ...
      receivePort.listen((message) {
        ...
        if (response.error != null) {
          ...
          try {
            final jsonMap = jsonDecode(response.snapshotJsonStr!)
